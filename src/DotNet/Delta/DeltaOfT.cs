...
namespace Stize.DotNet.Delta
{
    ...
    public class Delta<T> : TypedDelta where T : class
    {
        ...
        /// The derived object type or complex type for which the changes would be tracked.
        ...
        /// properties. <c>null</c> means this object type is not open.
        /// </param>
        ...
        /// <summary>
        /// Copies the changed property values from the underlying object (accessible via <see cref="GetInstance()" />)
        /// to the <paramref name="original" /> object recursively.
        /// </summary>
        /// <param name="original">The object to be updated.</param>
        public void CopyChangedValues(T original)
        {
            ...
            if (!this.structuredType.IsAssignableFrom(original.GetType()))
            {
                ...
                throw new ArgumentException(
                    $"Cannot use Delta of type '{this.structuredType.FullName}' on an object of type '{original.GetType().FullName}'");
            }
        ...
        /// <summary>
        /// Copies the unchanged property values from the underlying object (accessible via <see cref="GetInstance()" />)
        /// to the <paramref name="original" /> object.
        /// </summary>
        /// <param name="original">The object to be updated.</param>
        ...
        /// <summary>
        /// Overwrites the <paramref name="original" /> object with the changes tracked by this Delta.
        ...
        /// <summary>
        /// Overwrites the <paramref name="original" /> object with the values stored in this Delta.
        ...
        private static void CopyDynamicPropertyDictionary(IDictionary<string, object> source,
            IDictionary<string, object> dest, PropertyInfo dynamicPropertyInfo, T targetObject)
        {
            ...
            else
            {
                if (dest == null)
                {
                    dest = GetDynamicPropertyDictionary(dynamicPropertyInfo, targetObject, true);
                }
                else
                {
                    dest.Clear();
        ...
        private static IDictionary<string, object> GetDynamicPropertyDictionary(PropertyInfo propertyInfo, T targetObject, bool create)
        {
            ...
            if (create)
            {
                ...
                {
                    //throw Error.InvalidOperation(SRResources.CannotSetDynamicPropertyDictionary, propertyInfo.Name, targetObject.GetType().FullName);
                    throw new InvalidOperationException(
                        $"The dynamic dictionary property '{propertyInfo.Name}' of type '{targetObject.GetType().FullName}' cannot be set. The dynamic property dictionary must have a setter.");
                }

                IDictionary<string, object> newPropertyValue = new Dictionary<string, object>();

                propertyInfo.SetValue(targetObject, newPropertyValue);
        ...
        private void Reset(Type structuralType)
        {
            ...
            {
                throw new InvalidOperationException($"The actual targetObject type '{structuralType}' is not assignable to the expected type '{typeof(T)}'.");
            }
        ...
        private void CopyChangedDynamicValues(T targetObject)
        {
            if (this.dynamicDictionaryPropertyInfo == null)
            {
                return;
            ...
            var toDictionary =
                GetDynamicPropertyDictionary(this.dynamicDictionaryPropertyInfo, targetObject, false);
            ...
            CopyDynamicPropertyDictionary(tempDictionary, toDictionary, this.dynamicDictionaryPropertyInfo, targetObject);
        }

        // Missing dynamic structural properties MUST be removed or set to null in *Put*
        private void CopyUnchangedDynamicValues(T targetObject)
        {
            if (this.dynamicDictionaryPropertyInfo == null)
            {
                return;
            ...
            var toDictionary = GetDynamicPropertyDictionary(this.dynamicDictionaryPropertyInfo, targetObject, false);
